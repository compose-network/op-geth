...
func (miner *Miner) generateWork(genParam *generateParams, witness bool) *newPayloadResult {
	...
	for _, tx := range genParam.txs {
		...
		if err != nil {
			return &newPayloadResult{
				err: fmt.Errorf(
					"failed to force-include tx: %s type: %d sender: %s nonce: %d, err: %w",
					tx.Hash(),
					tx.Type(),
					from,
					tx.Nonce(),
	...
	if err != nil {
		...
		if backend, ok := miner.backendAPI.(BackendWithSequencerTransactions); ok {
			backend.OnBlockBuildingComplete(work.rpcCtx, nil, false, genParam.simulation)
		}
		return &newPayloadResult{err: err}
	}
	...
	if backend, ok := miner.backendAPI.(BackendWithSequencerTransactions); ok {
		backend.OnBlockBuildingComplete(work.rpcCtx, block, true, genParam.simulation)
	}
...
func (miner *Miner) makeEnv(
	...
	return &environment{
		...
		evm: vm.NewEVM(
			core.NewEVMBlockContext(header, miner.chain, &coinbase, miner.chainConfig, state),
			state,
			miner.chainConfig,
			vm.Config{},
		),
...
func (miner *Miner) commitTransactions(
	...
	for {
		...
		if env.gasPool.Gas() < ltx.Gas {
			log.Trace(
				"Not enough gas left for transaction",
				"hash",
				ltx.Hash,
				"left",
				env.gasPool.Gas(),
		...
		if isCancun {
			...
			if left < int(ltx.BlobGas/params.BlobTxBlobGasPerBlob) {
				log.Trace(
					"Not enough blob space left for transaction",
					"hash",
					ltx.Hash,
					"left",
					left,
		...
		if ltx.DABytes != nil && miner.config.MaxDABlockSize != nil {
			...
			if daBytesAfter.Cmp(miner.config.MaxDABlockSize) > 0 {
				log.Debug(
					"adding tx would exceed block DA size limit",
					"hash",
					ltx.Hash,
					"txda",
					ltx.DABytes,
		...
		case errors.Is(err, errTxConditionalInvalid):
			...
			tx.SetRejected()
			log.Warn(
				"Skipping account, transaction with failed conditional",
				"sender",
				from,
				"hash",
				ltx.Hash,
...
func (miner *Miner) commitAccountBasedTransactions(
	interrupt *atomic.Int32,
	env *environment,
	accountTxs map[common.Address][]*txpool.LazyTransaction,
) int {
	committed := 0
